...
export function SignalsTable({ signals, loading }: SignalsTableProps) {
  ...
  if (signals.length === 0) {
    return <div className="text-white/50">No signals for today</div>;
  }

  return (
    <div className="bg-[rgba(38,38,45,0.6)] backdrop-blur-md border border-white/[0.08] rounded-3xl p-6">
      <h3 className="text-xl font-semibold text-white mb-4">Today&apos;s Signals</h3>
...
function StatusBadge({ outcome }: { outcome: string | null }) {
  if (!outcome) {
    return <span className="px-3 py-1 rounded-full text-xs font-medium bg-orange-500/20 text-orange-400">Pending</span>;
  }
  if (outcome === 'win') {
    return <span className="px-3 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400">Win</span>;
  }
  return <span className="px-3 py-1 rounded-full text-xs font-medium bg-red-500/20 text-red-400">Loss</span>;
}
